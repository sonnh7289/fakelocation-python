...
cred = firebase_admin.credentials.Certificate('fakelocation1-c0453-firebase-adminsdk-aug09-56c714bd51.json')
...
async def post_img(file: UploadFile = File(...)):
    ...
    if (str(file.filename)[-3:] == 'png') or (str(file.filename)[-3:] == 'jpg') or (
    (str(file.filename)[-4:] == 'jfif')) or (str(file.filename)[-4:] == 'jpeg'):
        # save file to local
        con = engine.connect()
...
async def update_img(id: int, file: UploadFile = File(...)):
    ...
    (str(file.filename)[-4:] == 'jfif')) or (str(file.filename)[-4:] == 'jpeg'):
        ...
        con = engine.connect()
        data = con.execute(
            cut_change.update().values(img_name=file.filename, url_name=f"{IMAGEDIR}{file.filename}").where(
                cut_change.c.id == id))
        con.commit()
        return {
            "success": True,
...
async def update_img(id: int, file: UploadFile = File(...)):
    ...
    (str(file.filename)[-4:] == 'jfif')) or (str(file.filename)[-4:] == 'jpeg'):
        ...
        con = engine.connect()
        data = con.execute(
            change_img.update().values(img_name=file.filename, url_name=f"{IMAGEDIR_BG}{file.filename}").where(
                change_img.c.id == id))
        con.commit()
        return {"File :": "select files only .png, .jpg, .jfif",
                "filename": file.filename,
    ...
    for i in data:
        ...
        if ((name_img == '.png') or (name_img == '.jpg') or (name_img == 'fifj')) or (name_img == 'jpeg'):
            ...
            for i in bg_img:
                ...
                if ((name_img_bg == '.png') or (name_img_bg == '.jpg') or (name_img_bg == 'fifj')) or (
                        name_img_bg == 'jpeg'):
...
if __name__ == '__main__':
    FLASKapp.run(host='0.0.0.0', port=3001)