...
def signupAccount():
    ...
    try:
        ...
        mycursor.execute("SELECT MAX(id_user) from user")
        result = mycursor.fetchone()
        max_id_user = result[0] if result[0] else 0
...
def send_verification_email(email):
    ...
    from_address = "nguyentienmanh181102@gmail.com"  # Thay bằng email của bạn
    ...
    linkverify = firebase_admin.auth.generate_email_verification_link(
...
def save_user_to_mysql(
    ...
    connection = mysql.connector.connect(**config)
    mycursor = connection.cursor()
    sql = "INSERT INTO user (id_user, link_avatar, full_name, user_name, ip_register, device_register, password, email) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
    val = (
        id_user,
        link_avatar,
        full_name,
...
@app.route("/get1000comments", methods=["POST"])
